//! A latency measurement for Milestone J, not a second implementation of the library.
//!
//! Milestone J's own criterion is "a single-binary MPC step within the latency budget", which is a
//! measurement nobody had taken -- the crate that once lived here was deleted precisely because it
//! was an unmeasured reimplementation. So this mirrors exactly the slice the measurement needs and
//! nothing more: RK4 discretisation of a linear known system, the hand-written discrete adjoint,
//! and projected-gradient optimal control over a box, matching `chc.control` step for step.
//! `parity_check.py` pins the trajectory against the Python reference so the two are timed doing
//! the same arithmetic; `cargo bench`-free by design, since `hyperfine` times whole processes.
//!
//! It is outside `src/chc`, so `hatchling` never packages it and the published wheel is unchanged.
...
mod tests {
    ...
    fn optimize_control_reaches_python_optimum() {
        ...
        let x0 = dvector![1.0, 0.0];
        let xt = dvector![0.0, 0.0];
        let (_us, history) = optimize_control(
            &a, &b, &q, &r, &qf, &xt, &x0, 0.1, -5.0, 5.0, 30, 400, 0.2, 1e-9,
        );
